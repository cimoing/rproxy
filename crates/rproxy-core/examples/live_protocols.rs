...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (label, protocol) in tests {
        let Some(node) = config.nodes.iter().find(|node| node.protocol == protocol) else {
            println!("{label}=missing");
            continue;
        };

        let mut test_config = config.clone();
        test_config.profile.active_node = Some(node.id.clone());
        let http_listen = test_config.proxy.http_listen;
        let mut runtime = ProxyRuntime::new(test_config);
        runtime.start().await?;
        let result = test_connect(http_listen, &target, &host, port).await;
        runtime.stop().await;

        match result {
            Ok(status) => println!("{label}=ok {status}"),
    ...
    S: AsyncRead + Unpin,
{
    ...
    loop {
        match stream.read(&mut buffer).await {
            ...
            Err(error) if !body.is_empty() && error.to_string().contains("close_notify") => break,
